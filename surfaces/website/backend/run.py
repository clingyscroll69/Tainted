...
def apply_deployment_defaults() -> None:
    ...
    every `prove` would have gone back to running untrusted exploits in this process while
    looking exactly like a deployment that was working.
    ...
    `setdefault` throughout, never assignment: `=0` is the documented way to say out loud that
    you accept in-process execution, or that you want to watch the policy rather than enforce
    it, and a default you cannot override is not a default. The bundled demo contacts nothing
