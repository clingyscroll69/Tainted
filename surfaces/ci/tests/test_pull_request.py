...
# --------------------------------------------------------------------------- #
def test_pr_body_shows_the_request_that_proved_it():
    f = finding()
    body = pr_body([f], [fix_result(f)])
    assert "GET http://localhost:54321/rest/v1/notes" in body
    assert "Owner-scoped SELECT policy." in body


def test_pr_body_claims_no_verification_it_did_not_run():
    """The run that opens the PR proved against the unfixed app; a re-prove there only re-finds
    the hole. The body says where the proof comes from instead of printing a verdict."""
    f = finding()
    body = pr_body([f], [fix_result(f)])
    assert "own preview deploy is the proof of the fix" in body
