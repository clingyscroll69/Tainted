...
| `TAINTED_LOCKOUT` | `true` checks that this change did not lock the **owner** out of their own data. **A locked-out owner fails the job** — a change that secures the data by making it unreachable has not shipped a working feature. Needs a target, `TAINTED_LOGIN_A` and `TAINTED_SEED`. The owner is asked through the route the code shows serving that record, found from the seed's table. |
